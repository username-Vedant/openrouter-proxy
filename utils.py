...
def check_rate_limit_error(response: Response) -> Tuple[bool, Optional[int]]:
    ...
    if "X-RateLimit-Reset" in response.headers:
        try:
            ...
            logger.info(f"Found X-RateLimit-Reset in headers: {reset_time_ms}")
        ...
        try:
            ...
            if (
                ...
                if (
                    ...
                    try:
                        reset_time_ms = int(data["error"]["metadata"]["headers"]["X-RateLimit-Reset"])
                        logger.info(f"Found X-RateLimit-Reset in response metadata: {reset_time_ms}")
                    except (ValueError, TypeError) as e:
                        logger.warning(f"Failed to parse X-RateLimit-Reset from metadata: {e}")
            ...
            try:
                ...
                if data_part and data_part != '[DONE]':
                    ...
                    if (
                        ...
                        if "metadata" in data_json["error"] and "headers" in data_json["error"]["metadata"]:
                            headers = data_json["error"]["metadata"]["headers"]
                            if "X-RateLimit-Reset" in headers:
                                try:
                                    reset_time_ms = int(headers["X-RateLimit-Reset"])
                                    logger.info(f"Found X-RateLimit-Reset in SSE data: {reset_time_ms}")
                                except (ValueError, TypeError) as e:
                                    logger.warning(f"Failed to parse X-RateLimit-Reset from SSE: {e}")
                        break
